...
function Skpi() {
  ...
  useEffect(() => {
    const unvalidateSkpi = skpiData.filter((skpi) => skpi.status === "accepted by WD");
    const validatedSkpi = skpiData.filter((skpi) => skpi.status !== "pending" && skpi.status !== "accepted by WD");
        ...
        {error ? (
          ...
          <div>
            ...
            {showSuccessPopup && (
              <div className="fixed top-0 left-0 z-50 flex items-center justify-center w-full h-full bg-black bg-opacity-50">
                <div className="relative max-w-screen-lg p-6 mx-auto bg-white rounded-lg" style={{ width: "50vw" }}>
                  <h3 className="text-green-700 underline text-bold">Sukses</h3>
                  <div className="p-5">
                    <p>Kegiatan berhasil divalidasi!</p>
                    <div className="flex justify-end mt-4">
                      <button onClick={() => setShowSuccessPopup(false)} className="px-4 py-1 text-[16px] text-white rounded bg-amber-500 hover:bg-amber-700">
                        Tutup
                      </button>
                    </div>
                  </div>
          ...
          <div className="fixed top-0 left-0 flex items-center justify-center w-full h-full bg-black bg-opacity-50">
            <div className="relative max-w-screen-lg p-6 mx-auto bg-white rounded-lg" style={{ width: "50vw" }}>
              ...
              <div className="p-5">
                ...
                <button onClick={handleValidationConfirmation} className="px-4 py-1 mt-4 text-[16px] text-white rounded bg-amber-500 hover:bg-amber-700">
                  Validasi Kegiatan
                </button>
              </div>
            </div>
