...
function PertukaranMahasiswa() {
  ...
  const handleExports = async () => {
    try {
      const response = await axios.get("http://localhost:5001/api/v1/exports/noncompetitions/pertukaran/faculty", {
        headers: {
      ...
      link.href = fileURL;
      link.setAttribute("download", "Pertukaran_Mahasiswa.xlsx"); // Atur nama file yang ingin diunduh
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:5001/api/v1/achievements/noncompetitions/faculty", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  return (
    <div className="pt-6">
      <h2 className="font-semibold text-gray-900 font-poppins">Pertukaran Mahasiswa</h2>
      <div className="h-screen p-8 mt-8 shadow-boxShadow">
        ...
        <div className="mt-4">
          <select id="tahun" className="p-2 mt-5 bg-white border rounded-md border-secondary" value={selectedYear} onChange={handleYearChange}>
          ...
          {data
            ...
            .map((achievement) => (
              <div className="p-4 transition duration-300 bg-white border rounded-lg shadow-md border-secondary hover:shadow-lg" key={achievement.id}>
                ...
                <p className="text-gray-700">
                  <span className="text-lg font-semibold text-gray-900">Jurusan: </span>
                  <br /> <span> - {achievement.major}</span>
                </p>
              </div>
            ))}
        </div>
