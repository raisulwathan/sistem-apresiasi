      ...
      <div className="h-screen p-10 overflow-auto mt-9 shadow-boxShadow">
        ...
        {currentItems.length > 0 ? (
          <table className="min-w-full">
            <thead>
              <tr>
                <th className="px-4 py-2 font-medium text-white bg-secondary">Nama Kegiatan</th>
                <th className="px-4 py-2 font-medium text-white bg-secondary">Bidang Kegiatan</th>
                <th className="px-4 py-2 font-medium text-white bg-secondary">Tingkat</th>
                <th className="px-4 py-2 font-medium text-white bg-secondary">Tahun</th>
                <th className="px-4 py-2 font-medium text-white bg-secondary">Pemilik</th>
              </tr>
            </thead>
            <tbody>
              {currentItems.map((item, index) => (
                <tr key={item.id} className={index % 2 === 0 ? "bg-gray-100" : "bg-white"}>
                  ...
                  <td className="px-4 py-2 text-base">{item.level_activity}</td>
                  <td className="px-4 py-2 text-base">{item.participant_type}</td>
                  <td className="px-4 py-2 text-base">{item.year}</td>
        ...
        <div className="flex justify-center mt-4">
          <button className="px-4 py-2 mr-2 text-base border rounded-md hover:bg-dimBlue hover:border-white border-secondary" onClick={() => paginate(currentPage - 1)} disabled={currentPage === 1}>
            Previous
          </button>
          <button className="px-4 py-2 ml-2 text-base border rounded-md hover:bg-dimBlue hover:border-white border-secondary" onClick={() => paginate(currentPage + 1)} disabled={indexOfLastItem >= data.length}>
            Next
          </button>
        </div>
