...
class UsersService {
    ...
    async verfyUserCredential(npm, password) {
        ...
        if (!user) {
            const mhsUsk = await this.getUserFromWebService(npm)
    ...
    async getUserFromWebService(npm) {
        const mhsUsk = await axios.get(
            `http://ws.usk.ac.id/webservice/ws_siba/cSiba/mhs/npm/${npm}/key/021infsiba`
        )
