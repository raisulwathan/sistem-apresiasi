...
export const postAchievementIndependentController = async (req, res) => {
  ...
  const { name, levelActivity, participantType, totalParticipants, participants, faculty, major, achievement, mentor, year, startDate, endDate, fileUrl } = req.body;
...
export const getAchievementIndependentByFacultyController = async (req, res) => {
  const users = await usersService.getUserById(req.userId);
  const achievements = await achievementIndependentService.getAchievementIndependentByFaculty(users.faculty);
