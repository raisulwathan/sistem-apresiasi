...
export const getExportAchievementIndependentsController = async (req, res) => {
  ...
  if (userRole === 'OPERATOR') {
    ...
    const achievementData =
      await achievementIndependentService.getAchievementIndependentByFacultyForExport(
        users.faculty
      );

    const achievementDataWithoutIds = achievementData.map(
      ({ id, ...rest }) => rest
    );
  ...
  if (userRole === 'ADMIN') {
    const achievementData =
      await achievementIndependentService.getAchievementIndependentForExport();
...
export const getExportAchievementNonCompetitionByCategoryController = async (
  req,
  res
) => {
  const { userId, userRole } = req;
  ...
  if (userRole === 'OPERATOR') {
    ...
    const achievementDataWithoutIds = filteredAchievementData.map(
      ({ id, category, ...rest }) => rest
    );
    ...
    worksheet.columns = [
      { header: 'Nama Kegiatan', key: 'name', width: 50 },
      { header: 'Kegiatan', key: 'activity', width: 50 },
      { header: 'Fakultas', key: 'faculty', width: 20 },
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=non-competition_${category}_export.xlsx`
    );
