...
export const postSkpiController = async (req, res) => {
  ...
  const organizationPoints = activityPoints["Bidang Organisasi Kemahasiswaan dan Kepemimpinan"] || 0;
  const talentPoints = activityPoints["Bidang Minat, Bakat, Mental Spritiual Kebangsaan dan Kewirausahaan"] || 0;
  const scientificPoints = activityPoints["Bidang Penalaran dan Keilmuan, Penyelarasan dan Pengembangan Karir"] || 0;
  ...
  if (status !== "rejected") {
    switch (userRole) {
      ...
      case "WR":
        ...
        if (!ttd) {
          throw new InvariantError("TTD belum ditambahkan. Tambahkan TTD untuk menyelesaikan SKPI");
        }
