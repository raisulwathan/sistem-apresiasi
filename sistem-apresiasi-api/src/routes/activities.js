...
router.get(
  '/rejects',
  authenticateToken,
  tryCatch(getRejectActivitiesController)
);
router.get(
  '/points',
  authenticateToken,
  tryCatch(getActivitiesPointsController)
);
router.get(
  '/faculties',
  authenticateToken,
  tryCatch(getActivitiesByFacultyController)
);
router.get('/:id', authenticateToken, tryCatch(getActivityByIdController));
router.put(
  '/:id/validate',
  authenticateToken,
  tryCatch(putStatusActivityController)
);

router.get(
  '/:id/rejects',
  authenticateToken,
  tryCatch(getRejectActivityByIdController)
);
